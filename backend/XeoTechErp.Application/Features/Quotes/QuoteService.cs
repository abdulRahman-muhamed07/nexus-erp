...
public sealed class QuoteService(IQuoteRepository repository, IOrderRepository orderRepository, IUnitOfWork unitOfWork) : IQuoteService
{
    ...
    public async Task<QuoteDto> GetByIdAsync(int id, CancellationToken cancellationToken = default) => ToDto(await repository.GetByIdAsync(id, cancellationToken) ?? throw new KeyNotFoundException("Quote not found."));

    public async Task<QuoteDto> CreateAsync(CreateQuoteRequest request, CancellationToken cancellationToken = default)
    {
        if (request.CustomerId <= 0 || request.Items.Count == 0) throw new ArgumentException("Customer and items are required.");
        ...
        var quote = new Quote { CustomerId = request.CustomerId, Date = DateTime.UtcNow, DiscountPct = Math.Clamp(request.DiscountPct, 0, 100) };
        foreach (var item in request.Items)
        {
            if (item.Qty <= 0 || item.Price < 0) throw new ArgumentException("Invalid quantity or price.");
            quote.Items.Add(new QuoteItem { ProductId = item.ProductId, Name = products[item.ProductId].Name, Qty = item.Qty, Price = item.Price });
        }
    ...
    {
        var quote = await repository.GetByIdAsync(id, cancellationToken) ?? throw new KeyNotFoundException("Quote not found.");
        if (quote.Status != QuoteStatus.Approved) throw new InvalidOperationException("Quote must be approved first.");
    ...
    private static QuoteDto ToDto(Quote quote) => new(quote.Id, quote.CustomerId, quote.Status, quote.Date, quote.Subtotal, quote.Tax, quote.Shipping, quote.Total, quote.DiscountPct);
}
